exports.run = {
   ...
   async: async (m, {
      ...
      if (!args || !args[0]) return client.sendList(m.chat, '', `*Status* : [ ${setting[type] ? 'ON' : 'OFF'} ]`, '', 'Tap here!', [{
         rows
      }], m)
      let option = args[0].toLowerCase()
      let optionList = ['on', 'off']
      if (!optionList.includes(option)) return client.sendList(m.chat, '', `Status* : [ ${setting[type] ? 'ON' : 'OFF'} ]`, '', 'Tap here!', [{
         rows
      }], m)
      let status = option != 'on' ? false : true
      if (setting[type] == status) return client.reply(m.chat, Func.texted('bold', `${Func.ucword(command)} telah ${option == 'on' ? 'activated' : 'inactivated'} sebelumnya.`), m)
      setting[type] = status
